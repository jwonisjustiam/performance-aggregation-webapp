...
- 입력 SKU 구분 도구는 페이지 본문의 날짜·시간 필터를 선택적으로 적용하고 SKU 목록 기준으로 Basic, 웨어러블, 모바일 ACC를 분리합니다.
- Basic은 워치9 40mm(SM-L340/SM-L345), 워치9 44mm(SM-L350/SM-L355), 울트라2(SM-L715)로 구분합니다.
- 세 결과의 주문번호는 쉼표·하이픈 등 기호를 제거하고 숫자만 남깁니다.
- 결과 화면은 Basic/웨어러블/모바일 ACC 탭으로 나뉘며, Basic에만 워치 모델 구분을 표시합니다.
...
- `naver-live-stats-collector` Chrome 확장 프로그램으로 네이버 로그인 쿠키를 외부에 저장하지 않고 방송별 `시청/알림 통계` 박스의 시청수와 유니크 결제자수를 CSV로 수집할 수 있습니다.
- 외장하드는 `삼성공식파트너 쇼마젠시`, 웨어러블은 `삼성파트너 쇼마젠시`를 사용합니다. 통계 CSV를 앱에 올리면 방송별 `시청/알림 통계` 박스의 시청수와 결제 통계의 유니크 결제자수가 회차별로 매칭됩니다.
- 라이브 일정별 결과는 방송 정보 17열, 목표 3열, 실적 4열, 성과 2열, AI라이브 대응 2열의 총 28열입니다.
- 목표 View(만)/수량/금액(백만)은 `0.2`/`100.0`/`24.0` 고정값이고, 실적 View(만)/수량/전환율/금액(백만)은 앱이 계산합니다.
- 달성률은 `실적 금액(백만) / 목표 금액(백만) * 100` 수식이며, 사용자 입력 항목과 비용률은 빈칸으로 둡니다.
- 입력 SKU 구분 도구 결과는 Basic, 웨어러블, 모바일 ACC 파일을 각각 다운로드합니다.
- 2026-07-23 기준 DOCX 규칙을 반영했습니다.
- Docker/시놀로지 전용 파일은 제거했습니다.
- 입력 SKU 구분 처리 스모크 테스트와 전체 Python 문법검사를 통과했습니다.
