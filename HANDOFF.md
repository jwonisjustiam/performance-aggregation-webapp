...
- `naver-live-stats-collector` Chrome 확장 프로그램으로 네이버 로그인 쿠키를 외부에 저장하지 않고 방송별 라이브중 시청수와 유니크 결제자수를 CSV로 수집할 수 있습니다.
- 외장하드는 `삼성공식파트너 쇼마젠시`, 웨어러블은 `삼성파트너 쇼마젠시`를 사용합니다. 통계 CSV를 앱에 올리면 View(만)과 전환율이 회차별로 매칭됩니다.
