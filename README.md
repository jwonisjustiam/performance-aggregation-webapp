...
- 네이버 쇼핑라이브 통계 수집기 CSV 연결: 방송별 `시청/알림 통계` 박스의 `시청수`로 실적 `View(만)`을 채우고, 결제 통계의 유니크 결제자수/시청수로 실적 전환율 계산

### 라이브 일정별 결과 규칙

- 목표 `View(만)`, `수량`, `금액(백만)`은 각각 `0.2`, `100.0`, `24.0`으로 고정합니다.
- 실적 `View(만)`, `수량`, `전환율`, `금액(백만)`은 통계 CSV와 주문 Raw Data로 계산합니다.
- `달성률`은 `실적 금액(백만) / 목표 금액(백만) * 100`이며 엑셀 수식으로 저장합니다.
- 재방송여부부터 비고까지의 사용자 입력 항목과 비용률은 빈칸으로 둡니다.
- AI 여부, 제작(대행사), 출연자1은 각각 `AI`, `KCI`, `AI`로 출력합니다.
