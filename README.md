...
- 실적 전환율은 `실적 수량 / (실적 View(만) * 10000)` 엑셀 수식이며 `0.00%` 서식을 적용합니다. 현재 배치에서는 `V3/(U3*10000)`에 해당합니다.
- 다운로드 엑셀은 예외 처리 없이 `=V3/(U3*10000)` 수식을 사용합니다. 시청수가 0이거나 공란이면 `#DIV/0!`가 표시됩니다.
- `달성률`은 공란으로 둡니다.
- 재방송여부, 삼성 담당자, 거래선2, 비고, 비용률, 달성률은 빈칸으로 둡니다.
