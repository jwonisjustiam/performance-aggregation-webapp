...
def render_usage_guide() -> None:
    with st.expander("사용 안내", expanded=False):
        st.markdown(
            ...
            2. 업무 유형은 `라이브 SKU 구분 도구`, `라이브 일정별 구분 도구`, `입력 SKU 구분 도구` 3가지입니다.
...
def main() -> None:
    ...
    if job_type in {"samsung", "weekly"} and uploaded_files:
        with st.expander("회차 시간 설정", expanded=True):
            ...
            st.caption(
                "현재 접속 중 임시 저장한 시간 템플릿은 라이브 SKU 구분 도구와 라이브 일정별 구분 도구에서 공통으로 사용됩니다."
            )
            try:
                    ...
                    with save_col:
                        if st.button(
                            ...
                            st.success(
                                ...
                                "라이브 SKU 구분 도구와 라이브 일정별 구분 도구에서 불러올 수 있습니다."
                            )
                    with download_col:
                        st.download_button(
                            "현재 시간표 JSON 다운로드",
